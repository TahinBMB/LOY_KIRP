...
#---------distribution of loy across risk group------




#-------multivar after adjusting for confounding factors-----


library(survival)
library(dplyr)

# 1. Extract LASSO coefficients at lambda.min (exclude intercept)
lasso_coef <- coef(cvfit, s = "lambda.min")
beta <- as.vector(lasso_coef)
names(beta) <- rownames(lasso_coef)
beta <- beta[beta != 0 & names(beta) != "(Intercept)"]  # remove intercept if present

# 2. Get selected genes
selected_genes <- names(beta)

# 3. Subset expression matrix: samples x genes (make sure expr_t is samples x genes)
expr_selected <- expr_t[, selected_genes, drop = FALSE]

# 4. Calculate linear predictor (risk score)
risk_score <- as.vector(as.matrix(expr_selected) %*% beta)
names(risk_score) <- rownames(expr_selected)  # sample names

# 5. Prepare clinical data
clinical_vars <- metadat %>%
  select(sample,
         age_at_diagnosis.diagnoses,
         stage = ajcc_pathologic_stage.diagnoses) %>%
  mutate(stage = factor(stage, ordered = TRUE))

# 6. Clean sample IDs for merging (replace '.' with '-')
surv_subset$sample <- gsub("\\.", "-", surv_subset$sample)
clinical_vars$sample <- gsub("\\.", "-", clinical_vars$sample)
names(risk_score) <- gsub("\\.", "-", names(risk_score))

# 7. Merge survival data with clinical vars
surv_clin <- surv_subset %>%
  select(sample, OS.time, OS) %>%
  inner_join(clinical_vars, by = "sample")

# 8. Filter surv_clin to samples with risk scores
surv_clin <- surv_clin %>% filter(sample %in% names(risk_score))


# 9. Arrange and add risk_score to surv_clin
surv_clin <- surv_clin %>% arrange(sample)
risk_score_ordered <- risk_score[surv_clin$sample]
surv_clin$risk_score <- risk_score_ordered

# 10. Make sure variables are numeric where needed
surv_clin$OS.time <- as.numeric(as.character(surv_clin$OS.time))
surv_clin$OS <- as.numeric(as.character(surv_clin$OS))
surv_clin$risk_score <- as.numeric(surv_clin$risk_score)
surv_clin$age_at_diagnosis.diagnoses <- as.numeric(as.character(surv_clin$age_at_diagnosis.diagnoses))
surv_clin$stage <- factor(surv_clin$stage, ordered = TRUE)


# Create simplified stage grouping
# Check current levels of original stage
levels(surv_clin$stage)  

# Keep stage as is but remove empty "" levels if any
surv_clin$stage_simple <- factor(surv_clin$stage)

# Optional: drop empty or invalid levels
surv_clin$stage_simple <- droplevels(surv_clin$stage_simple)

# Check counts
table(surv_clin$stage_simple)

# Make sure stage_simple is a factor (it seems it is)
surv_clin$stage_simple <- factor(surv_clin$stage_simple,
                                 levels = c("Stage I", "Stage II", "Stage III", "Stage IV"))

# Fit the Cox model
cox_model <- coxph(Surv(OS.time, OS) ~ risk_score + age_at_diagnosis.diagnoses + stage_simple,
                   data = surv_clin)

# Summary
summary(cox_model)

# Convert to unordered factor (nominal)

surv_clin$stage_simple <- factor(as.character(surv_clin$stage_simple),
                                 levels = c("Stage I", "Stage II", "Stage III", "Stage IV"))

# Check again if ordered
is.ordered(surv_clin$stage_simple)  # Should now be FALSE

# Fit Cox model again
cox_model_nominal <- coxph(Surv(OS.time, OS) ~ risk_score + age_at_diagnosis.diagnoses + 0 + stage_simple,
                           data = surv_clin)


summary(cox_model_nominal)
###1. Extract detailed coefficients table and save
# Extract coefficients table from summary
cox_sum <- summary(cox_model_nominal)
coef_table <- as.data.frame(cox_sum$coefficients)

# Add Hazard Ratio (exp(coef)) column explicitly if not present
coef_table$HR <- exp(coef_table$coef)

# Reorder/rename columns for clarity
coef_table <- coef_table %>%
  dplyr::select(HR, coef, `se(coef)`, z, `Pr(>|z|)`)

# Rename columns nicely
colnames(coef_table) <- c("Hazard_Ratio", "Coefficient", "Std_Error", "Z_value", "P_value")
...
coef_table <- coef_table[, c("Variable", "Hazard_Ratio", "Coefficient", "Std_Error", "Z_value", "P_value")]

# Save to CSV
write.csv(coef_table, file = "cox_model_nominal_coefficients.csv", row.names = FALSE)


##2. Extract overall model statistics and save
overall_stats <- data.frame(
  Concordance_Index = round(cox_sum$concordance[1], 3),
  Concordance_SE = round(cox_sum$concordance[2], 3),
  Likelihood_Ratio_ChiSq = cox_sum$logtest["test"],
  Likelihood_Ratio_p = cox_sum$logtest["pvalue"],
  Wald_ChiSq = cox_sum$waldtest["test"],
  Wald_p = cox_sum$waldtest["pvalue"],
  Score_ChiSq = cox_sum$sctest["test"],
  Score_p = cox_sum$sctest["pvalue"],
  AIC = AIC(cox_model_nominal)
)

# Optional: transpose for better CSV layout
overall_stats_t <- data.frame(
  Metric = rownames(overall_stats),
  Value = as.numeric(overall_stats[,1])
)

# Save overall stats
write.csv(overall_stats_t, file = "cox_model_nominal_overall_stats.csv", row.names = FALSE)
#forest plot
...
coef_table$HR <- exp(coef_table$coef)
coef_table$lower95 <- exp(coef_table$coef - 1.96 * coef_table$`se(coef)`)
coef_table$upper95 <- exp(coef_table$coef + 1.96 * coef_table$`se(coef)`)

# Clean up variable names for display
clean_varnames <- function(varnames) {
  varnames <- gsub("stage_simple", "", varnames)
  varnames <- gsub("age_at_diagnosis.diagnoses", "Age at diagnosis", varnames)
  varnames <- gsub("risk_score", "Risk score", varnames)
  varnames
}
...
tabletext <- cbind(
  c("Variable", clean_varnames(rownames(coef_table))),
  c("HR (95% CI)", format_hr_ci(coef_table$HR, coef_table$lower95, coef_table$upper95)),
  c("p-value", format_pval(coef_table$`Pr(>|z|)`))
)

# Values for the plot
mean <- c(NA, coef_table$HR)
lower <- c(NA, coef_table$lower95)
upper <- c(NA, coef_table$upper95)
...
forestplot(
  ...
  title = "Forest plot of Cox model after adjusting Confounding Factors",
  graph.pos = 2,  # Position of the graph (column 2)
  log = TRUE,
  xticks = c(0.1, 1, 10, 100),  # Custom x-axis ticks for log scale
  clip = c(0.1, 100),  # Clip very large CIs
  txt_gp = fpTxtGp(
    label = gpar(cex = 0.9),
    ticks = gpar(cex = 0.8),
    xlab = gpar(cex = 1),
  ...
  hrzl_lines = list(
    "2" = gpar(lty = 2, col = "gray"),
    "7" = gpar(lty = 1, col = "black")
  )
)
###----multivar by risk group__---
library(survival)
library(forestplot)
library(dplyr)

# Fit Cox model with risk_group (Low as reference) and other covariates
cox_model_risk <- coxph(Surv(OS.time, OS) ~ risk_group + age_at_diagnosis.diagnoses + stage_simple,
                        data = surv_clin)

summary(cox_model_risk)

# Extract coefficients and compute HR, 95% CI
cox_sum <- summary(cox_model_risk)
coef_table <- as.data.frame(cox_sum$coefficients)
coef_table$HR <- exp(coef_table$coef)
coef_table$lower95 <- exp(coef_table$coef - 1.96 * coef_table$`se(coef)`)
coef_table$upper95 <- exp(coef_table$coef + 1.96 * coef_table$`se(coef)`)
...
tabletext <- cbind(
  c("Variable", rownames(coef_table)),
  c("HR (95% CI)", paste0(
    formatC(coef_table$HR, format = "f", digits = 2), " (",
    formatC(coef_table$lower95, format = "f", digits = 2), "-",
    formatC(coef_table$upper95, format = "f", digits = 2), ")"
  )),
  c("p-value", signif(coef_table$`Pr(>|z|)`, 3))
)
...
forestplot(
  ...
  col = fpColors(box = "royalblue", line = "darkblue"),
  xlab = "Hazard Ratio (95% CI)",
  title = "Forest Plot: Cox Model with Risk Group",
  graph.pos = 2,
  log = TRUE,
  ...
  hrzl_lines = list("2" = gpar(lty = 2, col = "gray"))
)
forestplot(
  labeltext = tabletext,
  mean = c(NA, coef_table$HR),
  ...
  graph.pos = 2,
  log = TRUE,
  xticks = c(0.1, 1, 10, 100),  # Updated ticks
  clip = c(0.1, 100),           # Expanded clip range
  txt_gp = fpTxtGp(
    label = gpar(cex = 0.9),
    ticks = gpar(cex = 0.8),
    xlab = gpar(cex = 1),
...
# Split hallmark_selected into separate vectors for each pathway
pathway_list <- hallmark_selected %>%
  group_by(gs_name) %>%
  summarise(genes = list(unique(gene_symbol))) %>%
  deframe()

# For clarity
mitotic_spindle_genes <- pathway_list[["HALLMARK_MITOTIC_SPINDLE"]]
e2f_target_genes <- pathway_list[["HALLMARK_E2F_TARGETS"]]
g2m_genes <- pathway_list[["HALLMARK_G2M_CHECKPOINT"]]
...
venn.plot <- venn.diagram(
  x = list(
    DEGs = deg_gene_names,
    Mitotic_Spindle = mitotic_spindle_genes,
    E2F_Targets = e2f_target_genes,
    G2M_Checkpoint = g2m_genes
  ),
  filename = NULL,  # so it returns a grid object instead of writing to file
...
# Combine the three pathways into one gene set
all_pathway_genes <- unique(c(mitotic_spindle_genes, e2f_target_genes, g2m_genes))

# Two-set Venn: DEGs vs pathway genes
venn.plot2 <- venn.diagram(
  x = list(
    DEGs = deg_gene_names,
    Pathways = all_pathway_genes
  ),
  filename = NULL,
...
plot_venn_with_title <- function() {
  grid.newpage()
  grid.draw(venn.plot2)
  grid.text("Overlap of DEGs and Selected Hallmark Pathways",
            y = unit(0.95, "npc"),
            gp = gpar(fontsize = 16, fontface = "bold"))
  grid.text("Pathways: Mitotic Spindle, E2F Targets, G2M Checkpoint",
            y = unit(0.90, "npc"),
            gp = gpar(fontsize = 12, fontface = "italic"))
}
plot_venn_with_title()
